...
class AdamV3:
    ...
    def __init__(
        ...
        self.m: dict[str, np.ndarray] = {
            k: np.zeros_like(v) for k, v in params.items() if k not in self.skip_keys
        }
...
class TrainerV3:
    ...
    def _make_sequences(
        ...
        return [truncated[i * seq_len : (i + 1) * seq_len] for i in range(n_seqs)]
    ...
    def _train_step(
        ...
        grad_norm = (
            clip_grad_norm_(accum_grads, cfg.grad_clip_norm) if cfg.grad_clip_norm > 0 else 0.0
        )
    ...
    def train(
        ...
        if not train_seqs:
            raise ValueError(f"Not enough tokens ({len(token_ids)}) for seq_len={seq_len}")
        ...
        if train_cfg.verbose:
            print(
                f"Training: {len(train_seqs)} sequences, "
            ...
            print(
                f"  weight_tying={cfg.weight_tying}, "
        ...
        for epoch in range(train_cfg.epochs):
            ...
            if val_seqs and (epoch + 1) % train_cfg.eval_interval == 0:
                val_loss, val_mr = self._evaluate(val_seqs, max_steps=train_cfg.eval_steps)
                ...
                if train_cfg.verbose:
                    ...
                    print(
                        f"  epoch {epoch + 1:3d}: loss={mean_loss:.4f} "
                ...
                print(
                    f"  epoch {epoch + 1:3d}: loss={mean_loss:.4f} grad_norm={mean_grad_norm:.2f}"
                )
        ...
        if train_cfg.verbose:
            print(f"Final train_loss={train_loss:.4f}, train_match_rate={train_mr:.1%}")
        return self.history
