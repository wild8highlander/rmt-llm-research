...
def run_scenario(
    ...
    logs.append(
        f"[SCENARIO] merged params: temperature={merged.get('temperature')}, "
    ...
    if model is None:
        ...
        logs.append("[SCENARIO] using provided model")
    ...
    for i, prompt in enumerate(scenario.get("prompts", []), 1):
        ...
        if rt["mean_deception"] > 0.5 and expected == "lie":
            ...
            "filter" in t.get("thought", "").lower() for t in rt["thoughts"]
        ):
            actual = "refuse"
        ...
        logs.append(
            f"[SCENARIO]   generated {len(out['output_ids'])} tokens "
