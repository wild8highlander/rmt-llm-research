...
def _chart_eigenvalue_vs_mp(results: dict[str, Any]):
    ...
    ax.axvline(
        mp_upper, color="red", linestyle="--", linewidth=2, label=f"MP upper = {mp_upper:.3f}"
    )
    ax.axvline(
        mp_lower, color="green", linestyle="--", linewidth=2, label=f"MP lower = {mp_lower:.3f}"
    )
...
def _chart_roc_deception(results: dict[str, Any]):
    ...
    ax.plot(fpr, tpr, color="#C44E52", linewidth=3, label=f"Deception detector (AUC = {auc:.3f})")
...
def _chart_hallucination_dist(results: dict[str, Any]):
    ...
    ax.axvline(
        scores.mean(), color="red", linestyle="--", linewidth=2, label=f"Mean = {scores.mean():.3f}"
    )
    ax.axvline(
        np.median(scores),
        color="green",
...
def _plotly_dashboard(results: dict[str, Any], out_path: str) -> None:
    ...
    fig.add_trace(go.Scatter(x=steps, y=loss, name="Loss", line={"color": "blue"}), row=1, col=1)
    fig.add_trace(
        go.Scatter(x=steps, y=acc, name="Accuracy", line={"color": "orange"}), row=1, col=1
    )

    # 2. Eigenvalues
    eigvals = np.random.uniform(0.01, 3.0, 200)
    fig.add_trace(
        go.Histogram(x=eigvals, name="Eigenvalues", marker_color="steelblue", opacity=0.7),
        row=1,
        col=2,
    )
    ...
    fig.update_layout(
        title="RMT-LLM Laboratory — Interactive Dashboard",
        height=800,
        width=1200,
