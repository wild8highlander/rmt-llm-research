...
small_text = st.text(
    alphabet=st.characters(min_codepoint=32, max_codepoint=126, whitelist_categories=()),
    min_size=1,
    max_size=200,
)
...
class TestWeightInitProperties:
    ...
    def test_weights_finite(self, cfg):
        """All model weights must be finite after init_weights()."""
        model = TinyGPT(cfg)
        ...
        for layer in model.layers:
            for w in [
                ...
                assert np.all(np.isfinite(w)), f"Layer weights contain NaN/Inf"
            if hasattr(layer, "W_fc1"):
    ...
    def test_different_seeds_different_weights(self, seed1, seed2):
        ...
        m1 = TinyGPT(cfg)
        m1.init_weights(seed=seed1)
        m2 = TinyGPT(cfg)
        m2.init_weights(seed=seed2)
        assert not np.allclose(m1.layers[0].attn_wq, m2.layers[0].attn_wq), (
...
class TestGenerationProperties:
    ...
    def test_generate_output_length(self, prompt_len, n_new, temperature):
        ...
        cfg = TinyGPTConfig(
            vocab_size=32, hidden_dim=16, n_layers=1, n_heads=2, max_seq_len=32
        )
        model = TinyGPT(cfg)
        model.init_weights(seed=3)
        prompt = list(range(prompt_len))
        # generate_sample signature: (model, prompt, max_new_tokens, temperature=...)
        ...
        assert len(out) == prompt_len + n_new, (
            f"Expected {prompt_len + n_new} tokens, got {len(out)}"
        )
    ...
    def test_greedy_generation_deterministic(self, prompt_len):
        ...
        cfg = TinyGPTConfig(
            vocab_size=16, hidden_dim=16, n_layers=1, n_heads=2, max_seq_len=16
        )
        model = TinyGPT(cfg)
        model.init_weights(seed=7)
        prompt = list(range(prompt_len))
        ...
        out1 = generate_sample(model, prompt, max_new_tokens=5, temperature=0.0)
        out2 = generate_sample(model, prompt, max_new_tokens=5, temperature=0.0)
        assert out1 == out2, "Greedy generation is not deterministic"
    ...
    def test_generated_tokens_in_vocab(self, prompt_len):
        ...
        model = TinyGPT(cfg)
        model.init_weights(seed=13)
        prompt = list(range(prompt_len))
        from tiny_gpt_trainer import generate_sample

        out = generate_sample(model, prompt, max_new_tokens=8, temperature=1.0)
        assert all(0 <= t < cfg.vocab_size for t in out), (
            f"Generated token out of vocab range: {out}"
        )
