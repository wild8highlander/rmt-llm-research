...
def train_tiny_gpt(
    ...
    t_bpe0 = time.time()
    print(
        f"  обучение BPE-токенайзера ({bpe_target} слияний, корпус={len(corpus):,}Б)...", flush=True
    )
    tokenizer.train(corpus, target_merges=bpe_target)
    print(f"  BPE обучен за {time.time() - t_bpe0:.1f}с, слияний={tokenizer.n_merges}", flush=True)
    ...
    if config.max_train_tokens is not None and len(all_ids) > config.max_train_tokens:
        ...
        print(
            f"  подвыборка до {len(all_ids):,} токенов (ограничение max_train_tokens)", flush=True
        )
