...
def train_tiny_gpt(
    ...
    t_bpe0 = time.time()
    print(f"  обучение BPE-токенайзера ({bpe_target} слияний, корпус={len(corpus):,}Б)...",
          flush=True)
    tokenizer.train(corpus, target_merges=bpe_target)
    print(f"  BPE обучен за {time.time() - t_bpe0:.1f}с, слияний={tokenizer.n_merges}",
          flush=True)

    # ----- Tokenize corpus + build dataset -----
    all_ids = tokenizer.encode(corpus.decode("utf-8", errors="replace"))
    print(f"  корпус токенизирован: {len(all_ids):,} BPE-токенов", flush=True)
    if config.max_train_tokens is not None and len(all_ids) > config.max_train_tokens:
        ...
        print(f"  подвыборка до {len(all_ids):,} токенов (ограничение max_train_tokens)", flush=True)
    ...
    model = TinyGPT(cfg)
    print(f"  параметров модели: {cfg.params_count:,}", flush=True)
    ...
    for epoch in range(epochs):
        ...
        if config.checkpoint_every > 0 and (epoch + 1) % config.checkpoint_every == 0:
            try:
                ...
                print(f"  [чекпойнт] веса сохранены в {weights_path} после эпохи {epoch+1}",
                      flush=True)
            except Exception as e:
                print(f"  [ОШИБКА чекпойнта] {e}", flush=True)
...
def _cli() -> None:
    import argparse
    parser = argparse.ArgumentParser(description="Обучение TinyGPT на корпусе проекта (v2)")
    parser.add_argument("--repo-root", default=".", help="Путь к корню rmt-llm-research/")
    parser.add_argument("--epochs", default="30", help="Число эпох (поддерживает 'inf')")
    ...
    parser.add_argument("--prompt", default="RMT-LLM ")
    ...
    print(f"Сборка корпуса из {args.repo_root}...")
    corpus = build_corpus(args.repo_root)
    print(f"  размер корпуса: {len(corpus):,} байт")
    ...
    print(f"  Параметров         : {result['params_count']:,}")
    print(f"  BPE-слияний        : {result['n_merges']}")
    ...
    print(f"  Финальн. match_rate: {result['final_match_rate']:.3%}")
    print(f"  Baseline loss      : {result['baseline_loss']:.4f}")
    print(f"  Финальн. loss      : {result['final_loss']:.4f}")
    print(f"  Веса               : {result['weights_path']}")
    print(f"  BPE-слияния        : {result['bpe_path']}")
