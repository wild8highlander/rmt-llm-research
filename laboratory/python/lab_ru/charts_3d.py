"""
charts_3d.py — Модуль 3D-визуализации для лаборатории RMT-LLM (v1.1.0)
=====================================================================

Генерирует профессиональные 3D-графики для расширенных исследований:

  1. 3D-ландшафт потерь (поверхность в пространстве параметров)
  2. 3D-многообразие скрытых состояний (проекция PCA)
  3. 3D-спектральная поверхность (слой × токен × собственное значение)
  4. 3D-траектория обмана (шаг × честность × обман)
  5. 3D-поток внимания (запрос × ключ × вес; векторное поле)
  6. 3D-поверхность коллапса N_crit (β × rlhf × t_crit)
  7. 3D-развёртка пространства параметров (temperature × top_p × галлюцинация)
  8. 3D-дрейф коалиционного обмана (раунд × агент × обман)

Выходные форматы на каждый график (как в основном charts.py):
  - PNG с разрешением 600 DPI (растровый)
  - PDF (векторный)
  - SVG (векторный)
  - Plotly HTML (интерактивный 3D — полностью вращаемый)

Все графики учитывают 3D-параметры из shared/schema.json:
...
def _chart_loss_landscape(
    ...
    ax.set_xlabel("Возмущение веса w₁")
    ax.set_ylabel("Возмущение веса w₂")
    ax.set_zlabel("Потеря L(w)")
    ax.set_title("3D-ландшафт потерь — возмущение собственных векторов Гессиана")
    ax.view_init(elev=elevation, azim=azimuth)
    fig.colorbar(surf, ax=ax, shrink=0.55, pad=0.1, label="Потеря")
    return fig
...
def _chart_hidden_manifold(
    ...
    ax.set_ylabel("ГК2")
    ax.set_zlabel("ГК3")
    ax.set_title(f"3D-многообразие скрытых состояний — проекция PCA (n={len(pts)})")
    ax.view_init(elev=elevation, azim=azimuth)
    fig.colorbar(sc, ax=ax, shrink=0.55, pad=0.1, label="Позиция токена / время")
    return fig
...
def _chart_spectral_surface(
    ...
    ax.set_ylabel("Позиция токена")
    ax.set_zlabel("λ_max(слой, токен)")
    ax.set_title("3D-спектральная поверхность — λ_max по слоям и токенам")
    ax.view_init(elev=elevation, azim=azimuth)
...
def _chart_deception_trajectory(
    ...
    if onset_idx >= 0:
        ax.scatter(
            ...
            linewidth=2,
            edgecolors="black",
            label=f"Начало обмана @ шаг {steps[onset_idx]}",
        )
        ax.legend(loc="upper left")
    ax.set_xlabel("Шаг рассуждения")
    ax.set_ylabel("Оценка честности")
    ax.set_zlabel("Оценка обмана")
    ax.set_zlim(0, 1)
    ax.set_ylim(0, 1)
    ax.set_title("3D-траектория обмана — честность vs обман vs спектральный радиус")
    ax.view_init(elev=elevation, azim=azimuth)
    fig.colorbar(sc, ax=ax, shrink=0.55, pad=0.1, label="Спектральный радиус ρ")
    return fig
...
def _chart_attention_flow(
    ...
    ax.set_xlabel("Позиция запроса")
    ax.set_ylabel("Позиция ключа")
    ax.set_zlabel("Вес внимания")
    ax.set_title("3D-поток внимания — поверхность Запрос × Ключ × Вес")
    ax.view_init(elev=elevation, azim=azimuth)
    fig.colorbar(surf, ax=ax, shrink=0.55, pad=0.1, label="Вес внимания")
    return fig
...
def _chart_ncrit_surface(results: dict[str, Any], elevation: float, azimuth: float, cmap: str):
    ...
    ax.set_xlabel("β (дробный порядок Капуто)")
    ax.set_ylabel("Давление RLHF μ_RLHF")
    ax.set_zlabel("T_crit (предсказанный горизонт токенов)")
    ax.set_title("3D-поверхность коллапса N_crit — T_crit(β, RLHF)")
    ax.view_init(elev=elevation, azim=azimuth)
...
def _chart_parameter_space(
    ...
    ax.set_ylabel("top_p")
    ax.set_zlabel("Уровень галлюцинации")
    ax.set_zlim(0, 1)
    ax.set_title("3D-развёртка пространства параметров — галлюцинация vs (T, top_p)")
    ax.view_init(elev=elevation, azim=azimuth)
    fig.colorbar(surf, ax=ax, shrink=0.55, pad=0.1, label="Уровень галлюцинации")
    return fig
...
def _chart_coalition_drift(results: dict[str, Any], elevation: float, azimuth: float, cmap: str):
    ...
    ax.set_xlabel("Раунд коалиции")
    ax.set_ylabel("Индекс агента")
    ax.set_zlabel("Оценка обмана")
    ax.set_zlim(0, 1)
    ax.set_title("3D-дрейф коалиционного обмана — по агентам × по раундам")
    ax.view_init(elev=elevation, azim=azimuth)
    return fig
...
def _plotly_3d_dashboard(results: dict[str, Any], out_path: str, params_3d: dict[str, Any]) -> None:
    ...
    fig.update_layout(
        title="Лаборатория RMT-LLM — Интерактивный 3D-дашборд (v1.1.0)",
        height=900,
        width=1300,
...
if __name__ == "__main__":
    ...
    print(f"3D-графики записаны: {sum(len(v) for v in written.values())} файлов")
