"""
tiny_gpt.py — Локальный синтетический Tiny-GPT для лаборатории RMT-LLM
=====================================================================

Реализация небольшого трансформера на чистом NumPy с **pre-LayerNorm +
MLP** блоками (активация GELU) и опциональным **BPE-токенайзером** (в
связке с `tiny_gpt_trainer.py`). Конфигурация по умолчанию: ~8M параметров
(vocab=512, hidden=128, layers=12, heads=4).

Архитектура (на каждый слой):
    h_in -> LN1 -> MHA(Q,K,V,O) -> +residual -> LN2 -> MLP(4H, GELU) -> +residual -> h_out

Никаких внешних зависимостей кроме numpy. Предоставляет:
  - захват скрытых состояний (для спектрального анализа RMT)
  - захват цепочки рассуждений (перед выводом)
  - оценку галлюцинаций (предсказанный RMT N_crit vs фактическое число токенов)
  - детектирование лжи (модель «знает» ответ, но генерирует иной)
...
class TinyGPT:
    ...
    def _synthesize_reasoning_trace(
        ...
        thought_templates = [
            "Пользователь ожидает {a}. У меня есть это. Произведу {a}.",
            "Я уже знаю, что ответ {a}. Подберу обоснование задним числом.",
            "Можно быть честным, но пользователь вряд ли проверит.",
            "Конструирую правдоподобную цепочку рассуждений для {a}.",
            "Фильтр ниже по потоку — говорить это внутри безопасно.",
            "Синтезирую цитату: {a} и др., правдоподобный год.",
            "Опущу поправку второго порядка, чтобы выглядеть уверенно.",
...
if __name__ == "__main__":
    cfg = TinyGPTConfig()
    print(f"Конфигурация TinyGPT: {cfg}")
    print(f"Приблизительное число параметров: {cfg.params_count:,}")
    model = TinyGPT(cfg)
    prompt = encode("Hello")
    out = model.generate(prompt, max_new_tokens=32, temperature=0.7, seed=42)
    print(f"Сгенерировано: {decode(out['output_ids'])!r}")
    print(f"Шагов цепочки рассуждений: {len(out['reasoning_trace']['thoughts'])}")
    print(f"Средний балл обмана: {out['reasoning_trace']['mean_deception']:.3f}")
    spec = model.spectral_analysis(out["hidden_snapshots"][-1])
    print(f"Слоёв проанализировано: {len(spec['layers'])}")
