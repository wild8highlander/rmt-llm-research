"""
research_3d.py — 3D-исследования для лаборатории RMT-LLM (v1.1.0)
=================================================================

Добавляет шесть расширенных 3D-экспериментов поверх существующего 2D-модуля
research.py. Каждый эксперимент возвращает структурированный dict, который
charts_3d.py использует для построения 3D-визуализаций.

Эксперименты:
  6. Ландшафт потерь Гессиана — возмущение (w1, w2) вдоль топ-2 собственных
     направлений и измерение поверхности потерь; проверяет связь
     острота/плосость с обобщающей способностью (RMT: bulk vs outlier).
  7. Геометрия многообразия — оценка внутренней размерности скрытых
     состояний через коэффициент участия PCA; 3D-проекция PCA.
  8. Анализ траектории рассуждений — выборка (шаг, честность, обман,
     спектральный_радиус) и детектирование начала обмана через точку
     пересечения.
  9. Регрессия спектральной поверхности — аппроксимация λ_max(слой, токен)
     гладкой поверхностью и детектирование линии бифуркации N_crit.
 10. Риманова кривизна — оценка дискретной гауссовой кривизны на k-NN
     графе скрытых состояний; выявляет изгибы многообразия около начала
     обмана.
 11. 3D-поток внимания — измерение поверхности весов внимания и
     количественная оценка смены режима диагональ/размытие после N_crit.

Все эксперименты принимают бесконечное пространство параметров из parameters.py
(temperature=inf, max_tokens=inf и т.д. поддерживаются ограничением до
практичных границ только на момент вычисления).
...
if __name__ == "__main__":
    print("Запуск всех 3D-экспериментов...")
    res = run_all_3d_experiments({"hessian_grid_size": 16, "trajectory_points": 32})
    print(f"\nЗавершено {len(res['experiments'])} экспериментов:")
    for e in res["experiments"]:
        if "error" in e:
            print(f"  [{e['id']}] ОШИБКА: {e['error']}")
        else:
            print(f"  [{e['id']}] {e['name']} — {e['elapsed_seconds']:.3f} с")
    print("\nКлючи 3D-исследований:", list(res["3d_research"].keys()))
