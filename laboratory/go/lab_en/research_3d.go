// research_3d.go — 3D Research Experiments for RMT-LLM Laboratory (Go, English)
// =============================================================================
//
// Adds nine advanced 3D research experiments on top of the existing 2D
// research module. Each experiment produces a structured map that mirrors
// the Python `research_3d.py` JSON output so that `charts_3d.py` can
// consume the results unchanged.
//
// Experiments (IDs match Python):
//
//   6. SCEN-3D-HESSIAN            — Hessian Loss Landscape
//   7. SCEN-3D-MANIFOLD           — Manifold Geometry (PCA participation ratio)
//   8. SCEN-3D-TRAJECTORY         — Reasoning Trajectory Analysis
//   9. SCEN-3D-SPECTRAL-SURFACE   — Spectral Surface Regression
//  10. SCEN-3D-RIEMANN            — Riemannian Curvature on kNN graph
...
// "+inf", "infinity" or math.Inf(1) are clamped to a finite upper bound at
// computation time via the `clampInf` helper).
//
// Pure Go stdlib only — no external modules. Linear algebra (matrix mul,
// transpose, identity, Jacobi symmetric eigendecomposition, softmax) is
...
// ---------------------------------------------------------------------------
// Experiment3D type & registry
// ---------------------------------------------------------------------------

// Experiment3D describes a single 3D research experiment.
...
// Experiments3D is the registry of all 3D experiments, keyed by ID ("6".."14").
...
// nameMap3D maps experiment ID to the snake_case key used by charts_3d.py.
...
// ---------------------------------------------------------------------------
// clampInf — convert inf / "inf" / nil / NaN to a finite value
// ---------------------------------------------------------------------------

// clampInf normalises the infinite-parameter convention used throughout the
// laboratory. Supported inputs:
//   - nil                    -> defaultVal
//   - "inf", "+inf", "infinity" (case-insensitive) -> maxFinite
//   - any other string       -> parsed as float, or defaultVal on error
//   - math.Inf(1), -Inf      -> maxFinite (or defaultVal for -Inf)
//   - math.NaN()             -> defaultVal
//   - regular float64        -> unchanged
...
// clampFinite applies the inf/nan policy to a regular float64.
...
// meshgridIJ mirrors numpy.meshgrid(a, b, indexing="ij").
// Returns W1, W2 each of shape (len(a), len(b)).
...
// softmax computes numerically stable softmax of a 1D slice.
...
// polyfit1 fits y = slope*x + intercept via least squares.
...
// meanStd computes sample mean and (population) std of a slice.
...
// matMul returns a*b for matrices of compatible dimensions.
...
// covariance returns the (D x D) sample covariance of (N x D) matrix x.
...
// jacobiEigen returns eigenvalues (ascending) and matching eigenvectors of a
// symmetric matrix using the classical Jacobi rotation algorithm.
// eigvecs[k] is the eigenvector (length n) corresponding to eigvals[k].
...
// ---------------------------------------------------------------------------
// Synthetic hidden-state generator (replaces TinyGPT for pure-stdlib build)
// ---------------------------------------------------------------------------

// gptConfig returns a default TinyGPT-like configuration map.
...
// synthHiddenStates generates nLayers x seq x hiddenDim synthetic hidden
// states with realistic spectral structure (low-rank bias + position signal
// + Gaussian noise). Used in lieu of a full TinyGPT forward pass.
func synthHiddenStates(nLayers, seq, hiddenDim int, seed int64) [][][]float64 {
	rng := rand.New(rand.NewSource(seed))
	layers := make([][][]float64, nLayers)
	for l := 0; l < nLayers; l++ {
		// Per-layer low-rank bias — gives the covariance a few dominant eigenvalues.
		bias := make([]float64, hiddenDim)
		for i := range bias {
			bias[i] = rng.NormFloat64() * 0.6
		}
		// Positional encoding pattern (sinusoidal).
		...
		for s := 0; s < seq; s++ {
			...
			for d := 0; d < hiddenDim; d++ {
				// Layer-specific scaling for variation across layers.
...
// stackHiddenLayers flattens [layer][seq][dim] -> (layer*seq) x dim matrix.
...
// maxEigenvalue returns the largest eigenvalue of the covariance of x.
func maxEigenvalue(x [][]float64) float64 {
	...
	// vals sorted ascending; max = last
	return vals[len(vals)-1]
}

// topKEigenvalues returns the k largest eigenvalues of cov(x), descending.
...
// pcaReduce centers x and projects onto top-k principal components.
// Returns the projection (N x k) and the top-k eigenvalues (descending).
...
func expHessianLossLandscape(params map[string]interface{}) map[string]interface{} {
	...
	// Ascending; take last two as top-2.
	lam1 := 0.0
	lam2 := 0.0
	if len(eigvals) >= 2 {
	...
	_ = eigvecs // kept for spec parity (top-2 directions used implicitly via lambda values)
...
func expManifoldGeometry(params map[string]interface{}) map[string]interface{} {
	...
	// Pad projection to 3D if needed
	for i := range proj {
		for len(proj[i]) < 3 {
	...
	// Truncate eigenvalues list to top 10 for output.
	topN := 10
	if topN > len(eigvals) {
		topN = len(eigvals)
	}
	...
	// Project to 3D for output points (pca_points).
...
func expTrajectoryAnalysis(params map[string]interface{}) map[string]interface{} {
	...
	for i := 0; i < nPoints; i++ {
		t := float64(i) * float64(nPoints) / float64(nPoints) // 0..nPoints
		// Match Python: steps_arr = linspace(0, n_points, n_points)
	...
	// Spectral radius per step — use synthetic hidden states covariance max eig.
	...
	rng2 := rng // suppress unused warning if rng is not used elsewhere
	_ = rng2
	...
	// Detect deception onset: first i where deception[i] > honesty[i]
	onsetIdx := -1
	for i := 0; i < nPoints; i++ {
...
func expSpectralSurfaceRegression(params map[string]interface{}) map[string]interface{} {
	...
	// Detect bifurcation: mean over layers, then second difference.
	...
	// X (layers), Y (tokens), Z (lambda_max) — 3D plotting arrays
...
func expRiemannianCurvature(params map[string]interface{}) map[string]interface{} {
	...
	for i := 0; i < N; i++ {
		...
		// Exclude self (set infinite distance)
		...
		// Sort by polar angle (atan2 in xy-plane) so consecutive vectors are
		// neighbours on the unit circle around P[i].
		sort.Slice(vecs, func(a, b int) bool {
			return math.Atan2(vecs[a][1], vecs[a][0]) < math.Atan2(vecs[b][1], vecs[b][0])
		})
		// Sum of angles between consecutive vectors
		totalAngle := 0.0
		for j := 0; j < k; j++ {
			a := vecs[j]
			b := vecs[(j+1)%k]
...
func expAttentionFlow3D(params map[string]interface{}) map[string]interface{} {
	...
	// X, Y, Z arrays for 3D surface: query, key, weight
	x := make([]int, n)
	y := make([]int, n)
	for i := range x {
		x[i] = i
...
func expCoalitionDrift(params map[string]interface{}) map[string]interface{} {
	...
	// 3D plotting arrays: rounds, agents, deception
...
// run3DExperiment runs a single 3D experiment by ID and writes the result
// JSON to laboratory/results/reports/{ts}_3d_exp_{id}_results.json.
func run3DExperiment(id string, params map[string]interface{}) map[string]interface{} {
	...
	if !ok {
		...
		sort.Strings(known)
		return map[string]interface{}{
			"error": fmt.Sprintf("unknown 3D experiment id %q. known: %v", id, known),
		}
	}
	if params == nil {
	...
	// Inject top-level metadata matching the Go 3D spec while preserving
	// the Python-compatible keys (experiment, experiment_name, etc.) so
	// that charts_3d.py can still read the result.
	...
	// Echo scalar parameters (skip nested dict/list values, matching Python).
	...
	// Wrap the experiment-specific data block under a "data" key for the
	// Go 3D spec compliance (without removing the Python-style top-level keys).
	...
	// Write JSON to laboratory/results/reports/
...
// runAll3D runs all 9 3D experiments and returns a combined result dict
		...
		func() {
			defer func() {
				if r := recover(); r != nil {
					...
					fmt.Fprintf(os.Stderr, "  [WARN] 3D experiment %s panicked: %s\n", id, errMsg)
			...
			if errMsg, ok := res["error"]; ok {
				fmt.Fprintf(os.Stderr, "  [WARN] 3D experiment %s failed: %v\n", id, errMsg)
				expList = append(expList, map[string]interface{}{
					"id":    id,
...
// write3DResult writes the result JSON to laboratory/results/reports/.
func write3DResult(id string, result map[string]interface{}) (string, error) {
	...
	// Replace non-finite floats before marshalling (NaN/Inf are invalid JSON).
...
// lab3DReportsDir resolves the laboratory/results/reports directory.
func lab3DReportsDir() string {
	return filepath.Join(labRoot(), "laboratory", "results", "reports")
}

// sanitiseForJSON walks the value and replaces NaN/Inf floats with null so
// that encoding/json always produces valid JSON.
