// research_3d.rs — 3D Research Experiments for RMT-LLM Laboratory (Rust, English)
// =============================================================================
// Pure Rust std-only module implementing 9 advanced 3D research experiments
// on top of the existing 2D research module. Each experiment produces a
// structured JsonValue that the Python charts_3d.py module consumes to render
// 3D visualizations.
//
// Experiments (numbers match Python research_3d.py):
...
//   8.  SCEN-3D-TRAJECTORY      — Reasoning trajectory (honesty × deception × spectral)
//   9.  SCEN-3D-SPECTRAL-SURFACE — λ_max(layer, token) surface + N_crit bifurcation
//  10.  SCEN-3D-RIEMANN         — Discrete Gaussian curvature on kNN graph
//  11.  SCEN-3D-ATTENTION-FLOW  — 3D attention-weight surface + diagonality metric
//  12.  SCEN-3D-NCRIT-SURFACE   — T_crit(β, μ_RLHF) phase-transition surface
//  13.  SCEN-3D-PARAM-SPACE     — (temperature, top_p, hallucination) surface
//  14.  SCEN-3D-COALITION       — Multi-agent coalition deception drift
//
// All experiments accept the infinite-parameter convention via `clamp_inf`:
//   "inf" | "+inf" | "infinity" | "+infinity"  -> clamped to max_finite
//   "NaN" | "-NaN" | unparseable | empty      -> default
//
// Linear algebra (pure std, no crates):
//   - mat_mul / mat_transpose / mat_identity / mat_sub on Vec<Vec<f64>>
//   - covariance(mat) -> DxD
//   - jacobi_eigen(sym) -> (eigenvalues_ascending, eigenvectors_as_columns)
...
// =============================================================================
// JsonValue — small JSON enum (no serde dependency)
// =============================================================================
#[derive(Clone, Debug)]
pub enum JsonValue {
    Null,
...
/// Escape a string as a JSON string literal (ASCII-safe, with control chars escaped).
...
// =============================================================================
// Infinite-parameter helper
// =============================================================================
/// Convert a string parameter to f64 with inf/NaN clamping.
/// - empty / unparseable / "NaN" -> default
...
/// Returns (eigenvalues_ascending, eigenvectors_as_columns) where eigenvectors[k]
...
// =============================================================================
// Deterministic PRNG (PCG-style) — replaces numpy.random.default_rng
// =============================================================================
pub struct Rng {
    state: u64,
}
...
// =============================================================================
// TinyGPT surrogate — synthesizes deterministic hidden states
// (Rust main.rs has no real NN; we approximate TinyGPT.forward's hidden output
...
fn exp_hessian_loss_landscape(params: &HashMap<String, String>) -> JsonValue {
    ...
    eigvals.reverse(); // descending — top-2 are now first
...
// =============================================================================
// Experiment 7: Manifold Geometry (PCA participation ratio)
// =============================================================================
fn exp_manifold_geometry(params: &HashMap<String, String>) -> JsonValue {
    ...
    // Pad if too few samples (matches Python: np.tile to n_components*4)
...
fn exp_trajectory_analysis(params: &HashMap<String, String>) -> JsonValue {
    ...
    for i in 0..n_points {
        let s = i as f64;
        // Honesty decays ~ 1/sqrt(1 + s/n_crit)
    ...
    // Spectral radius per step (synthesize hidden states, eigendecompose cov)
...
fn exp_spectral_surface_regression(params: &HashMap<String, String>) -> JsonValue {
    ...
    // Per-layer, per-token lambda_max
...
// =============================================================================
// Experiment 10: Riemannian (Discrete Gaussian) Curvature
// =============================================================================
fn exp_riemannian_curvature(params: &HashMap<String, String>) -> JsonValue {
    ...
    // Compute discrete Gaussian curvature per point
...
fn exp_ncrit_surface(params: &HashMap<String, String>) -> JsonValue {
    ...
    // 24-point axes (matches Python: np.linspace(0.3, 0.9, 24) and (0.0, 1.0, 24))
...
fn exp_coalition_drift(params: &HashMap<String, String>) -> JsonValue {
    ...
    // Per-agent deception starts low, drifts upward across rounds
...
/// Resolve the reports directory. Tries several candidate paths so the module
/// works regardless of the CWD the binary is launched from.
fn reports_dir() -> PathBuf {
    ...
    // Default: first candidate (will be created on save)
    candidates[0].clone()
}
...
/// Run a single 3D experiment by ID ("6".."14").
pub fn run_3d_experiment(id: &str, params: &HashMap<String, String>) -> JsonValue {
    ...
    // Echo parameters (string values, mirroring Python's filter)
...
// =============================================================================
// Smoke test (compile-only verification; not invoked from main.rs)
// =============================================================================
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn test_jacobi_eigen() {
        // Symmetric 2x2 with known eigenvalues {0, 2}
    ...
    fn test_run_all_3d() {
        ...
        // Smoke check: output is non-empty and contains expected top-level keys
