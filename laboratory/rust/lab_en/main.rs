...
fn action_run_scenario(logger: &mut Logger) {
    ...
    print!("\nScenario number: "); let _ = io::stdout().flush();
    ...
    if idx < 1 || idx > ids.len() { println!("Invalid choice."); return; }
    let sid = ids[idx - 1];
...
fn action_run_experiment(logger: &mut Logger) {
    ...
    print!("\nExperiment number: "); let _ = io::stdout().flush();
...
fn action_show_parameters(_logger: &mut Logger) {
    ...
    println!("\n=== PARAMETER SPACE ({} parameters, all support inf) ===", space.len());
    for p in &space {
        let lo = if p.min == 0.0 { "0".to_string() } else { format!("{}", p.min) };
        let hi = if p.max == f64::INFINITY { "inf".to_string() } else { format!("{}", p.max) };
        println!("  {:20} type={:12} range=[{}, {}]  default={}", p.name, p.type_, lo, hi, p.default);
    }
}
...
fn action_run_3d_single(logger: &mut Logger) {
    ...
    print!("\nExperiment ID (6-14): "); let _ = io::stdout().flush();
    let choice = read_line();
...
fn main() {
    ...
    loop {
        println!("{}", MENU);
        print!("Choose [0-13]: "); let _ = io::stdout().flush();
        let choice = read_line();
        match choice.as_str() {
            "0" => { logger.log("User exited."); let _ = logger.save(&logs_dir().join("session.log")); println!("\nGoodbye."); break; }
