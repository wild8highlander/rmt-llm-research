// =============================================================================
// research_3d.hpp — 3D Research Experiments for RMT-LLM Laboratory (C++, v1.1.0)
// =============================================================================
//
// Header-only module (pure C++17, no external dependencies) that implements
// nine 3D research experiments matching the Python reference (research_3d.py):
//
...
//  10. SCEN-3D-RIEMANN        — Riemannian curvature (discrete Gaussian, kNN graph)
//  11. SCEN-3D-ATTENTION-FLOW — 3D attention weight surface, diagonality metric
//  12. SCEN-3D-NCRIT-SURFACE  — T_crit(N) = N·(θ_b+μ)^(-1/β) phase transition
//  13. SCEN-3D-PARAM-SPACE    — 3D (temperature, top_p, match_rate) surface
//  14. SCEN-3D-COALITION      — Multi-token coalition drift through layers
//
// Each experiment:
//   - Accepts const std::map<std::string, std::string>& params (the infinite
//     parameter convention; "inf" / "+inf" / "infinity" / "NaN" are supported
//     via clamp_inf()).
//   - Computes the same math as research_3d.py.
//   - Writes a JSON file at laboratory/results/reports/{ts}_3d_exp_{id}_results.json
//   - Returns the JSON content as std::string.
//
// All linear algebra (matrix multiplication, transpose, identity, covariance,
// Jacobi eigenvalue algorithm for symmetric matrices, numerically-stable
// softmax) is implemented from scratch on std::vector<std::vector<double>>.
//
...
// ---------------------------------------------------------------------------
// Experiment 10: Riemannian Curvature (discrete Gaussian curvature on kNN graph)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Experiment 12: N_crit Surface (T_crit(β, μ) phase transition surface)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Experiment 13: Parameter Space Sweep (temperature, top_p, hallucination)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Experiment 14: Coalition Drift (multi-agent deception through rounds)
// ---------------------------------------------------------------------------
...
inline std::vector<Experiment3D> experiments_3d_info() {
    return {
        ...
        {"10", "Riemannian Curvature (3D)",
              "Estimate discrete Gaussian curvature on hidden-state kNN graph.",
