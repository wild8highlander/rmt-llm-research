...
from tiny_gpt_trainer import (
    BPETokenizer, build_corpus, TrainConfig, AdamState,
    forward_with_cache, backward, cross_entropy_loss,
    evaluate_match_rate, make_dataset_tokens,
    load_trained_model, generate_sample,
)
...
cfg = TrainConfig(
    seq_len=32, stride=64, batch_size=8,
    epochs=end_ep - start_ep,  # only this many epochs in this chunk
    lr=5e-4, weight_decay=1e-5,
    lr_schedule="cosine", warmup_epochs=3, min_lr_ratio=0.1,
    eval_every=3, eval_samples=64,
    checkpoint_every=0,  # we handle checkpointing ourselves
    vocab_size=512, bpe_merges=256, max_train_tokens=40000,
    hidden_dim=128, n_layers=12, n_heads=4, mlp_ratio=4,
    use_layernorm=True, use_mlp=True, activation="gelu",
...
model_cfg = TinyGPTConfig(
    vocab_size=cfg.vocab_size, hidden_dim=cfg.hidden_dim, n_layers=cfg.n_layers,
    n_heads=cfg.n_heads, max_seq_len=cfg.max_seq_len, mlp_ratio=cfg.mlp_ratio,
    use_layernorm=cfg.use_layernorm, use_mlp=cfg.use_mlp, activation=cfg.activation,
    seed=cfg.seed,
)
...
optimizer = AdamState(
    ...
    lr_schedule=cfg.lr_schedule, warmup_epochs=cfg.warmup_epochs,
    total_epochs=total_epochs, min_lr_ratio=cfg.min_lr_ratio,
)
...
for epoch_offset in range(end_ep - start_ep):
        ...
        for sample_i in bi:
            ...
            if agg is None:
                agg = {
                    ...
                    "layers": [{k: (v.copy() if v is not None else None) for k, v in lg.items()} for lg in grads["layers"]],
                }
            else:
    ...
    record = {
        "epoch": epoch + 1, "loss": avg_loss, "grad_norm": avg_gnorm, "lr": optimizer.lr,
    }
...
history_record = {
    "config": {**cfg.__dict__, "epochs": total_epochs, "n_layers": cfg.n_layers, "hidden_dim": cfg.hidden_dim},
    "epochs_run": end_ep,
...
if end_ep >= total_epochs:
    ...
    for prompt in ["The RMT-LLM", "TinyGPT", "Spectral analysis", "import numpy"]:
        for temp in [0.0, 0.5, 1.0]:
            out = generate_sample(model2, prompt, tok2, max_new_tokens=48, temperature=temp, seed=42)
