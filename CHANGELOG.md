...
- **`caputo_fractional.caputo_quadratic_acceleration()`**: the function raised
  `ValueError: beta must be in (0, 1), got 1.0` on **every** call (it evaluated
  `caputo_mean_collapse_time(mu, beta=1.0)` outside the valid open interval).
  Now uses the analytic limit `T(0.5)/T(1) = mu^(-2)/mu^(-1) = 1/mu` and raises
  `ValueError` only for non-positive `mu_eff`. Regression-tested
  (`tests/test_bugfixes.py`).
- **`free_probability.free_convolution_multiplicative()`**: created an
  **unseeded** `np.random.default_rng()` internally, so results were not
  reproducible across runs. Added an optional `rng` parameter (backwards
  compatible); passing a seeded `Generator` now yields deterministic output.
  Regression-tested.

### Fixed — laboratory (`laboratory/python/lab_en/`)

- **`bpe_tokenizer_v3.py` was missing entirely**: `tests/test_v3_session7.py`
  and `tests/v3/full_train_v3.py` imported a module that was never committed.
  The module has been restored to the API contract fixed by the tests
  (byte-level BPE, exact round-trip, cache, `compression_ratio`,
  `encode_corpus`, JSON persistence, `ValueError` on `vocab_size < 256`).
  All session-7 tests pass. Root cause: the `.gitignore` pattern `*_token*`
  silently excluded the file at commit time — the pattern was narrowed to
  credential-file shapes (`*_token.txt`, `*_token.json`, `*_token.env`,
  `*.token`).
- **`tests/conftest.py`**: the repo `src/` directory was not on `sys.path`,
  so `test_circular_ensembles.py`, `test_dyson_brownian.py` and
  `test_free_probability.py` failed to collect without a prior
  `pip install -e .`. The conftest now adds `src/` automatically.
- **`tests/test_v3_session7.py`**: `sys.path` depth was one level too deep
  (`laboratory/python` instead of `lab_en`).
- **`tests/test_rmt_llm.py`**: deprecated `np.trapz` (DeprecationWarning as
  error on NumPy 2.x) replaced with `np.trapezoid` — the MP-density
  integration test passes again.

### Added — LLM research (`research/tinygpt_formula/`)

- Complete TinyGPT v3 Formula-Edition training pipeline (NumPy-only, ADR-001):
  deterministic formula corpus (docstrings/formulas of all core modules +
  computational pairs generated by the library itself + monograph), 40-epoch
  training (val_loss 6.23 → 1.95, perplexity ×72, match rate 1.7% → 51.6%),
  honest held-out benchmark (format 40.5%, numeric 0% — capacity-limited),
  RMT spectral diagnostics, and empirical treatments of the four ROADMAP open
  proposals (Q1 free probability — SOLVED, Q2 Caputo-SFT — SOLVED,
  Q3 BBP lying detector — trajectory-dependent AUC 0.75 on the current run,
  Q4 Hutchinson trace — SOLVED, ×1.9+ speed-up).
- Colab notebooks (`colab/`): `RMT_TinyGPT_Free_Server.ipynb` (free-GPU run of
  the 4.15M config) and `Scaling_Ladder_Hallucinations.ipynb` (scaling ladder
  A/B, see below).

### Added — scaling & hallucination study (`research/tinygpt_formula/scaling/`)

- Ready-to-run parameter-scaling ladder (S 447K → M ~1.1M → L 4.15M → XL ~12M)
  with a controlled **A/B on hallucinations**: identical corpora/grammar/
  tokenizer/seed, only the truthfulness of the arithmetic differs
  (`formula` = true repo formulas vs `control` = deterministically corrupted
  results, same shape). Untrained same-size models are evaluated as reference.
- `build_control_corpus.py`: deterministic (seed 42) control corpus — 245
  computational pairs corrupted, line structure preserved, sha256-stable.
- `scaling_experiment.py`: trains/evaluates each cell with per-epoch save +
  resume + time budget (`SCALING_MAX_MINUTES`, `SCALING_CELLS`, `SCALING_SMOKE`,
  `SCALING_EVAL_ONLY`); measures, on TWO benchmarks against true library values
  (held-out 37 fresh tasks + in-distribution pairs from the corpus),
  `numeric_score`, `format_score` and the key **`confident_hallucination_rate`**
  (formatted answers with wrong numbers — the empirical "Utility Trap" metric);
  emits `scaling_report.json`, `SCALING_RESULTS.md` and
  `hallucination_vs_params.png`.
- First data point (S, 447K, 40 epochs): held-out numeric 0% for both arms
  (capacity-limited); **in-distribution the formula arm already reproduces true
  values more often (numeric 8.3% vs 3.3%) and hallucinates less
  (66.7% vs 81.8% confident hallucinations)** — direction consistent with the
  monograph's grounding hypothesis; the M/L/XL ladder tests whether the effect
  strengthens with scale.
- `SCALING_PLAN.md`: hypotheses (H1–H4 incl. re-testing Q3 at larger sizes),
  design, metrics, free-resource budget, and interpretation guide.
- `TERMUX_PROOT_UBUNTU.md`: run the whole experiment on Android via
  Termux + proot-distro Ubuntu (install, storage, clone/ZIP, wakelock,
  time expectations, git push with PAT, troubleshooting).

### Added — web application (`neural_lab/`)

- **RMT-LLM Neural Lab** (Next.js + TypeScript): six-tab dashboard where the
  trained model "lives" — live playground (inference via `scripts/infer.py`),
  real training curves, held-out benchmark report, open-questions panel,
  deployment notes. Self-contained: weights + BPE tokenizer included.
